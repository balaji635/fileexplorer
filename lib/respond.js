...
const respond = (request, response) => {
    ...
    let fileExt = path.extname(fullStaticPath);

    // ✅ Fix: Ensure getMimeType returns a valid Promise
    let mimePromise = getMimeType(fileExt);
    
    if (!mimePromise || typeof mimePromise.then !== 'function') {
        console.error("getMimeType() did not return a Promise!");
        response.writeHead(500, { 'Content-Type': 'text/plain' });
        response.write('500: Internal Server Error (MIME Type Issue)');
        return response.end();
    }
    ...
    mimePromise.then(mime => {
        ...
        // ✅ Stream all files, let the browser decide whether to display or download
